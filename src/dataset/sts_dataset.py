...
from src.input import StsInput
...
class StsDataset(Dataset):
    ...
    def loader(self, shuffle, batch_size):
    ...
    def collater(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        collated_labels = torch.zeros((batch_size, 1), dtype=torch.float)

        """ cut data if size > target_size else: fill by self.pad_token_id """
        for i, (input_id, attention_m, label, size) in enumerate(
                zip(a_input_ids, a_attention_mask, labels, sizes)):
        ...
        for i, (input_id, attention_m, label, size) in enumerate(
                zip(b_input_ids, b_attention_mask, labels,sizes)):
            diff = target_size - size
            if diff < 0:
